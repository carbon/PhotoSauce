...
use jxl::{
    api::{
        ...
        JxlOutputBuffer, JxlPixelFormat, JxlProgressiveMode, ProcessingResult,
...
fn read_image_info(
    ...
    loop {
        match decoder.process(input).map_err(|error| error.to_string())? {
...
fn scan_animated_image(input: &mut impl JxlBitstreamInput) -> Result<ScannedImage, String> {
    let mut decoder = read_image_info(input, decoder_options(true, false))?;

    while decoder.has_more_frames() {
        let frame_decoder = read_frame_header(decoder, input)?;
...
fn inspect_image_from_slice(data: &[u8]) -> Result<ScannedImage, String> {
    ...
    let (mut decoder, mut input_offset) =
        read_image_info_from_slice(data, decoder_options(false, true))?;
    if decoder.basic_info().animation.is_some() {
        let mut animation_input = data;
        return scan_animated_image(&mut animation_input);
    }
    ...
    if straight_alpha {
        (decoder, input_offset) = read_image_info_from_slice(data, decoder_options(false, false))?;
    }

    finish_still_image(decoder, input_offset)
}
...
fn inspect_image_from_reader<R: Read + Seek>(
    ...
    input
        ...
        .map_err(|error| error.to_string())?;
    let (mut decoder, mut input_offset) =
        read_image_info_from_reader(input, decoder_options(false, true))?;
    if decoder.basic_info().animation.is_some() {
        input
            .seek(SeekFrom::Start(0))
            .map_err(|error| error.to_string())?;
        return scan_animated_image(input);
    }
    ...
    if straight_alpha {
        ...
        (decoder, input_offset) =
            read_image_info_from_reader(input, decoder_options(false, false))?;
    }
...
fn decode_frame_from_slice(
    ...
    if let Some(seek_target) = frame.seek_target {
        let mut input = data;
        let mut decoder =
            read_image_info(&mut input, decoder_options(false, image.alpha_associated))?;
        ...
        let frame_decoder = read_frame_header(decoder, &mut input)?;
        let output_buffer = prepare_output(image, output, output_length, output_stride)?;
        return decode_current_frame(frame_decoder, &mut input, output_buffer);
    }
    ...
    let (mut decoder, mut input) = if let Some(primed) = take_primed_still(image) {
        ...
        let mut input = data;
        let decoder = read_image_info(&mut input, decoder_options(false, image.alpha_associated))?;
        (decoder, input)
    };
    configure_output_format(&mut decoder, image.info.channels);
    let frame_decoder = read_frame_header(decoder, &mut input)?;
    let output_buffer = prepare_output(image, output, output_length, output_stride)?;
    decode_current_frame(frame_decoder, &mut input, output_buffer)
}
...
fn decode_frame_from_reader<R: Read + Seek>(
    ...
    let decoder = if let Some(seek_target) = frame.seek_target {
        input
            .seek(SeekFrom::Start(0))
            .map_err(|error| error.to_string())?;
        let mut decoder = read_image_info(input, decoder_options(false, image.alpha_associated))?;
        configure_output_format(&mut decoder, image.info.channels);
        decoder.start_new_frame(seek_target);
        input
    ...
    let frame_decoder = read_frame_header(decoder, input)?;
    let output_buffer = prepare_output(image, output, output_length, output_stride)?;
    decode_current_frame(frame_decoder, input, output_buffer)
}
...
pub unsafe extern "C" fn jxlrs_inspect(
    ...
    with_ffi_result(|| {
        ...
        let image = inspect_image_from_slice(input)?;
...
pub unsafe extern "C" fn jxlrs_inspect_stream(
    ...
    with_ffi_result(|| {
        ...
        let image = inspect_image_from_reader(&mut input)?;
        // SAFETY: The caller guarantees that info is writable.
        unsafe { info.write(image.info) };
        Ok(Box::into_raw(Box::new(image)))
    })
...
mod tests {
    ...
    fn scans_and_decodes_basic_image() {
        let image = inspect_image_from_slice(BASIC_JXL).expect("scan");
    ...
    fn bounds_still_metadata_input() {
        ...
        let image = inspect_image_from_slice(&padded).expect("slice scan");
        ...
        let image = inspect_image_from_reader(&mut reader).expect("reader scan");
        let primed_offset = image
            .primed_still
            .lock()
            .expect("lock")
    ...
    fn rejects_truncated_input() {
        assert!(inspect_image_from_slice(&BASIC_JXL[..2]).is_err());
    }
}
